	...
	ImagesZoom.prototype={
		...
		init:function(params){
			...
			swiper2= new Swiper('#swiper2',{
		        ...
		        initialSlide:params.activeIndex?params.activeIndex:0,
		        speed:1000
			});
			//图片垂直居中
		...
		movePos: function(){
			...
			//$("#touchLog").html("self.isFinish:"+self.isFinish);
			//当右边滑动距离超过图片边缘一定距离时下一张图片
			if(self.moveX<self.wrapX-self.element.offsetWidth-30){
				if(!self.isFinish){
					self.isFinish=true;
					swiper2.slideNext();
			...
			}else if(self.moveX>30+self.element.offsetWidth-self.wrapX){
				if(!self.isFinish){
					self.isFinish=true;
					swiper2.slidePrev();
