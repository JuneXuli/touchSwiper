(function(window, undefined){
	...
	ImagesZoom.prototype = {
		...
		init: function(param){
			var self   = this,
				params = param || {};
			//querySelectorAll() 返回匹配的所有元素，没有则返回空的nodelist（节点数组）
			//querySelector()  返回匹配的第一个元素，没有则返回null
...
//				document.removeEventListener("touchmove", self.eventStop, false);
//			}, false);
			//给需要点击缩放的图片添加点击事件并初始化相关变量值
			for(var len=imgList.length,i=0; i<len; i++){
				imgList[i].addEventListener("click", function(){
					imgSrc = this.getAttribute("src");         //获取图片路径
					//cssText用来批量设置css样式
					zoomMask.style.cssText = "display:block";  //设置整个遮罩层可见
					zoomImg.src = imgSrc;                      //设置遮罩层中可缩放图片路径
					...
					zoomImg.onload = function(){
						...
						self.addEventStart({
							wrapX: zoomMask.offsetWidth,   //遮罩层宽（可看到的区域宽）
							wrapY: zoomMask.offsetHeight,  //遮罩层高（可看到的区域高）
